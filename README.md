...
- **Aggregation** ("has-a", independent lifecycle) — open diamond `◇` in UML *(AQA core)*
- **Composition** ("has-a", dependent lifecycle) — filled diamond `◆` in UML *(AQA core)*
- ASCII / UML class diagram notation
- "has-a" vs "is-a": choosing relationships over inheritance
- Real-world design examples
- Association ("uses-a") — *(supporting vocabulary for UML modelling; not a separate AQA headline term)*
